...
async def handle_home(request):
    """Serve HTML instructions page with a webcal:// link template."""
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  ...
  <style>
    ...
    .input-group input {{ flex:1; padding:10px; border:1px solid #ccc;
                          ...
                          font-size:1em }}
    .input-group button {{ padding:10px 20px; border:0; background:#1d72b8;
...
<body>
  <div class="container">
    ...
    <div class="input-group">
      ...
      <button onclick="navigator.clipboard.writeText('webcal://{host}/cal/{{YOUR_DISCORD_USER_ID}}.ics');alert('Copied');">
        Copy
      </button>
    </div>
    <div class="footer">
      &copy; {dt.datetime.now().year} Bot &nbsp;|&nbsp; Auto-refresh every {POLL_INTERVAL} min
    </div>
  </div>
</body>
</html>"""
...
async def handle_feed(request):
    """Serve the ICS feed file for the given user ID, or 404 if missing."""
    try:
